...
class GameEngine {
    ...
    update(deltaTime) {
        ...
        if (this.alienCharacter && this.inputHandler) {
            ...
            // Handle jump input for restart in Game Over state (tablet-friendly)
            if (this.gameState.currentState === GAME_STATES.GAME_OVER || 
                this.gameState.currentState === GAME_STATES.VICTORY) {
                const movement = this.inputHandler.getMovementInput();
                
                if (movement.jump && this.gameState.canRestart) {
                    console.log('Jump input detected in Game Over state - restarting game');
                    this.reset();
                }
            }
    ...
    renderEndGameState() {
        ...
        this.ctx.font = '14px Arial';
        this.ctx.fillText('Springe oder drücke R zum Neustarten', this.canvas.width / 2, this.canvas.height / 2 + 60);
        
        // Additional touch instruction for mobile users
        this.ctx.fillStyle = '#888888';
        this.ctx.font = '12px Arial';
        this.ctx.fillText('📱 Tippe zum Neustarten', this.canvas.width / 2, this.canvas.height / 2 + 80);
    }
    ...
    changeGameState(newState) {
        ...
        switch (newState) {
            ...
            case GAME_STATES.GAME_OVER:
                ...
                updateGameStatus('SPIEL VORBEI - Springe oder drücke R zum Neustarten', 'game-over');
                break;
            ...
            case GAME_STATES.VICTORY:
                ...
                this.gameState.canRestart = true;
                this.updateBestScore();
                updateGameStatus('SIEG! - Springe oder drücke R um nochmal zu spielen', 'victory');
                break;
