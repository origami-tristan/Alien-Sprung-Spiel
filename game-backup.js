...
const CHARACTER_CONFIG = {
    ...
    CAMERA_FOLLOW_SPEED: 0.1 // How smoothly camera follows
};
...
class Renderer {
    ...
    followTarget(targetX, targetY = 0) {
        ...
        this.camera.x += deltaX * this.camera.followSpeed;
    ...
    renderBackground() {
        ...
        for (let i = 0; i < 50; i++) {
            const baseX = (i * 73) % (this.canvas.width * 3); // Wider star field
            const x = (baseX - parallaxOffset) % (this.canvas.width + 100);
            const y = (i * 23) % (this.canvas.height * 0.6);
    ...
    renderUI(gameState, fps, alienCharacter = null, levelConfig = null) {
        ...
        // Game state info
        this.renderTextWithOutline(`State: ${gameState.currentState}`, 10, 25, '#ffffff', '#000000', '12px Arial');
        ...
        this.ctx.fillText(`Score: ${gameState.score}`, this.canvas.width - 10, 25);
        ...
        if (alienCharacter) {
            ...
            this.ctx.fillText(`Distance: ${Math.round(alienCharacter.position.x)}px`, 10, 60);
            ...
            this.ctx.fillText(`Next Level: ${Math.round(distanceToNextLevel)}px`, 10, 75);
        }
        
        // Difficulty indicator
        if (gameState.levelManager) {
            this.ctx.fillStyle = '#ff8800';
            this.ctx.fillText(`Difficulty: ${(gameState.levelManager.difficultyMultiplier || 1).toFixed(1)}x`, 10, 90);
        }
...
class LevelManager {
    constructor() {
        ...
        // Jump validation settings (more precise calculations)
        this.alienMaxJumpHeight = this.calculateMaxJumpHeight();
        this.alienMaxJumpDistance = this.calculateMaxJumpDistance();
    ...
    updateEndlessLevel(alienX, spikeObstacles) {
        ...
        if (this.lastGeneratedX < generationThreshold) {
            ...
            console.log(`Generated ${newSpikes.length} new spikes ahead`);
        }
    ...
    generateMoreSpikes(startX, endX) {
        ...
        // Test overall level completability
        const isCompletable = this.testLevelCompletability(levelConfig);
        if (!isCompletable) {
            console.warn('Level failed completability test, regenerating with safer parameters...');
            levelConfig.spikePositions = this.generateSaferLevel(obstacleCount);
        }
        
        console.log(`Level ${levelNumber} generated with ${levelConfig.spikePositions.length} spikes (Completable: ${isCompletable})`);
        return levelConfig;
    }
    
    // Generate a safer level when validation fails
    generateSaferLevel(count) {
        console.log('Generating safer level configuration...');
        
        const positions = [];
        const startX = CHARACTER_CONFIG.START_X + 80;
        const safeSpacing = Math.min(this.alienMaxJumpDistance * 0.7, 100); // 70% of max distance
        const safeHeight = Math.min(this.alienMaxJumpHeight * 0.6, 25); // 60% of max height
        
        for (let i = 0; i < count; i++) {
            const x = startX + (i * safeSpacing);
            const height = safeHeight + Math.random() * 5; // Small variation
...
class SpikeObstacle {
    ...
    render(ctx) {
        ...
        // Draw spike body with gradient effect
        ...
        // Draw spike shape (triangle)
        ctx.beginPath();
        ctx.moveTo(bounds.x + bounds.width / 2, bounds.y); // Top point
        ctx.lineTo(bounds.x, bounds.y + bounds.height - 8); // Bottom left
        ctx.lineTo(bounds.x + bounds.width, bounds.y + bounds.height - 8); // Bottom right
        ctx.closePath();
        ctx.fill();
        
        // Draw spike tip highlight
        ctx.fillStyle = `rgba(255, 255, 255, ${0.3 + this.glowIntensity * 0.4})`;
        ctx.beginPath();
        ctx.moveTo(bounds.x + bounds.width / 2, bounds.y);
        ctx.lineTo(bounds.x + bounds.width / 2 - 3, bounds.y + 6);
        ctx.lineTo(bounds.x + bounds.width / 2 + 3, bounds.y + 6);
        ctx.closePath();
        ctx.fill();
...
class GameEngine {
        ...
        if (this.alienCharacter && this.inputHandler) {
            ...
            if (this.isPlayable()) {
                ...
                if (this.levelManager && this.levelManager.isEndlessMode) {
                    this.levelManager.updateEndlessLevel(this.alienCharacter.position.x, this.spikeObstacles);
                }
    ...
    renderMenuState() {
        ...
        this.ctx.fillText('ALIEN JUMP GAME', this.canvas.width / 2, this.canvas.height / 2 - 50);
        ...
        if (this.stats.bestScore > 0) {
            this.ctx.fillText(`Best Score: ${this.stats.bestScore}`, this.canvas.width / 2, this.canvas.height / 2 + 50);
        }
    }
    ...
    renderGameplayState() {
        ...
        if (this.alienCharacter && this.alienCharacter.isInvulnerable) {
            ...
            this.ctx.font = '12px Arial';
            this.ctx.fillText('INVULNERABLE', this.canvas.width - 10, 50);
        }
    }
    ...
    renderEndGameState() {
        ...
        this.ctx.textAlign = 'center';
        const message = this.gameState.isVictory ? 'VICTORY!' : 'GAME OVER';
        ...
        if (this.gameState.score === this.stats.bestScore && this.stats.bestScore > 0) {
            this.ctx.fillStyle = '#ffaa00';
            this.ctx.font = '14px Arial';
            this.ctx.fillText('NEW BEST SCORE!', this.canvas.width / 2, this.canvas.height / 2 + 35);
        }
    ...
    renderAlienCharacter() {
        ...
        this.ctx.textAlign = 'left';
        this.ctx.fillText(`State: ${char.currentAnimation}`, bounds.x, bounds.y - 35);
        this.ctx.fillText(`Vel: ${Math.round(char.velocity.x)}, ${Math.round(char.velocity.y)}`, bounds.x, bounds.y - 25);
        this.ctx.fillText(`Frame: ${char.animationFrame}`, bounds.x, bounds.y - 15);
        this.ctx.fillText(`Grounded: ${char.isGrounded}`, bounds.x, bounds.y - 5);
    }
    ...
    renderCollisionDebug() {
        ...
        if (this.alienCharacter) {
            ...
            this.ctx.font = '10px Arial';
            this.ctx.textAlign = 'left';
            this.ctx.fillText(`Speed: ${Math.round(physicsInfo.speed * 10) / 10}`, 10, 100);
            this.ctx.fillText(`Energy: ${Math.round(physicsInfo.kineticEnergy * 10) / 10}`, 10, 115);
        }
    ...
    changeGameState(newState) {
        ...
        switch (newState) {
            ...
            case GAME_STATES.PAUSED:
                updateGameStatus('PAUSED - Press P to resume', 'paused');
                break;
            ...
            case GAME_STATES.GAME_OVER:
                ...
                updateGameStatus('GAME OVER - Press R to restart', 'game-over');
                break;
            ...
            case GAME_STATES.VICTORY:
                ...
                this.gameState.canRestart = true;
                this.updateBestScore();
                updateGameStatus('VICTORY! - Press R to play again', 'victory');
                break;
                
            case GAME_STATES.MENU:
                updateGameStatus('Press SPACE to start', '');
                break;
    ...
    completeLevel() {
        ...
        updateGameStatus(`Level ${this.levelManager.currentLevel} Reached! +${levelBonus} points`, 'victory');
    ...
    checkGroundCollision() {
        ...
        if (alienBottom >= groundLevel) {
            // Create a ground object for collision resolution
            const ground = {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (gameEngine.init()) {
        ...
        updateGameStatus('Initialization Failed', 'game-over');
    }
});
